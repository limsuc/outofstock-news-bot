...
자동 알림 운영은 Google Cloud Run functions + Cloud Scheduler를 사용합니다. 매일 17:03부터 17:43 KST까지 5분 간격으로 확인하고, Firestore 발송 이력으로 중복 알림을 방지합니다.
...
- `TELEGRAM_CHAT_ID` 텔레그램 chat id. 현재 운영 기본값은 `569967356`입니다.
- `OUTOFSTOCK_FILE_ID` 매일 갱신되는 품절 PDF의 Google Drive 파일 ID
- `SALES_LIST_FILE_ID` Google Drive에 올린 `sales-list.xlsx`의 파일 ID. 현재 운영 기본값은 `1ZZOH5A3AzDcHEyzoOx43KwSVxeLk5XXE`입니다.

## GitHub Actions 수동 실행

`.github/workflows/outofstock-alert.yml`은 수동 테스트용입니다. 자동 실행은 Google Cloud Scheduler가 담당합니다.
...
`sales-list.xlsx` 파일은 Google Drive에서 "링크가 있는 모든 사용자 보기 가능"으로 설정해야 자동 실행 환경에서 다운로드할 수 있습니다.
...
로컬/GitHub 수동 실행은 `data/alerts.sqlite3`에 발송 이력을 기록합니다. Google Cloud 자동 실행은 Firestore의 `sent_alerts` 컬렉션에 날짜별 발송 이력을 기록하여, 같은 품목도 매일 한 번 알리고 같은 날 반복 호출만 차단합니다.
수동 실행에서 `force_send=true`를 선택하면 로컬/GitHub 수동 실행의 중복 이력을 무시하고 현재 결과를 다시 발송합니다.
